  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-light)]">
      ...
      <button 
        ...
        className="fixed bottom-24 right-6 z-40 w-14 h-14 bg-[#ff6b00] hover:bg-[#e66000] text-white rounded-full shadow-2xl flex items-center justify-center transition-all"
      >
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="w-full max-w-sm bg-white h-full relative z-10 flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
                ...
                items.map(item => (
                  <div key={item.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg border border-gray-100">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-[#ff6b00]">₦{item.price.toLocaleString()}</p>
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200">
              ...
              <Link to="/checkout" className="block w-full text-center bg-[#ff6b00] hover:bg-[#e66000] text-white font-bold py-3 rounded-full transition-colors disabled:opacity-50">
        ...
        <aside className={`md:w-64 flex-shrink-0 ${isFilterOpen ? 'block' : 'hidden md:block'}`}>
          <div className="bg-white p-5 rounded-lg border border-[var(--color-border)] shadow-sm sticky top-24">
            <h3 className="font-bold text-lg mb-4 flex items-center justify-between">
              Filters
              <button className="text-sm font-normal text-[#ff6b00] hover:underline">Clear all</button>
            </h3>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                  {['All Categories', 'Tutoring', 'Design & Creative', 'Programming', 'Writing', 'Digital Marketing'].map(cat => (
                    <label key={cat} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="category" className="text-[#ff6b00] focus:ring-[#ff6b00]" defaultChecked={cat === 'All Categories'} />
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="number" placeholder="Min" className="w-full p-2 border border-gray-300 rounded-md text-sm outline-none focus:border-[#ff6b00]" />
                  <span className="text-gray-500">-</span>
                  <input type="number" placeholder="Max" className="w-full p-2 border border-gray-300 rounded-md text-sm outline-none focus:border-[#ff6b00]" />
                </div>
              </div>
        ...
        <main className="flex-1">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <select className="border border-gray-300 rounded-md py-1 px-2 bg-white outline-none focus:border-[#ff6b00]">
            ...
            {mockListings.map(listing => (
              <div key={listing.id} className="relative group">
                ...
                <button 
                  ...
                  className="absolute bottom-4 right-4 bg-[#ff6b00] hover:bg-[#e66000] text-white p-2 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
                >
